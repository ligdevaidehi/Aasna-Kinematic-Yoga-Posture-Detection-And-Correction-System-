...
class Movenet(object):
  ...
  def init_crop_region(self, image_height: int,
    ...
    if image_width > image_height:
      x_min = 0.0
      box_width = 1.0
      # Pad  vertical dimension to become  square image.
      y_min = (image_height / 2 - image_width / 2) / image_height
      box_height = image_width / image_height
    else:
      y_min = 0.0
      box_height = 1.0
      # Pad  horizontal dimension to become  square image.
  ...
  def _torso_visible(self, keypoints: np.ndarray) -> bool:
    ...
    right_hip_visible = right_h ip_score > Movenet._MIN_CROP_KEYPOINT_SCORE
  ...
  #def _run_detector(
      #self, image: np.ndarray, crop_region: Dict[(str, float)],
      #crop_size: (int, int)) -> np.ndarray:
    """Runs model inferenc  e on the cropped region.
